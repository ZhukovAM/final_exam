...
int main(int argc, char **argv) {
    //std::cout << "Server have benn started" << std::endl;

    // check arguments
//    if(argc < ){
//        std::cout << "Run with wrong options" << std::endl;
//        return 1;
//    }
...
//    addr.sin_addr.s_addr = htonl(INADDR_ANY);
    ...
    if(idDemon = fork()) {
        while (1) {
            // TODO выполняем что-нибудь в родителе
        }
    } else {
...
void http_GET(std::string request, struct ev_io *watcher, ssize_t r)
{
   // cout << "in http_get" << endl;
    ...
    size_t fs = request.find("/", 0);
    //cout << fs << endl;
    size_t ss = request.find(" ", fs);
    //cout << ss << endl;

    std::string fileName = request.substr(fs+1 , ss - fs - 1);
